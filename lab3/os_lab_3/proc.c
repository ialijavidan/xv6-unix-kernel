...
void run_first_level_processes()
{
  ...
  random = ticks;
  release(&tickslock);
  random_ticket = (random * 11 + 13) % max_ticket_number + 1;
...
void scheduler(void)
{
  ...
  for (;;)
  {
    ...
    for (p = ptable.proc; p < &ptable.proc[NPROC]; p++)
	{
		p->waitingCycle = cycleNum - p->cycleNum;
		if(p->level != 0){
			if(p->waitingCycle > 2500){
				p->level = 0;
			}
		}
	}
...
int intToStr(int arg0, char* str, int end) 
{ 
    int index = 0; 
