...
consoleintr(int (*getc)(void))
{
  ...
  while((c = getc()) >= 0){
    ...
    case TAB:
        ...
        for(int i = 0;getc() >= 0; i++)
          consoleReadBuf[i] = c;

        for(i = 0; i < strlen(*commandHistory) && !ifFound; i++)
          if(compare_strings(commandHistory[i], consoleReadBuf) == 0) {
            ifFound = 1;
            index = i;
        }
        ...
        if(index != -1){
          //show last command
          for(int i=0; i < strlen(commandHistory[index]); i++){
...
// int i;
// for(i = 0;!(recentCommands[i] && !recentCommands[0]) && i != 9;++i)
// {

//}
//if(i != 9) {
	//recentCommands[i-1] = dst;
	//strcpy(recentCommands[i-1], dst);
	//copy(recentCommands[i-1], dst, n);
//} else if (i == 9) {
	//recentCommands[1] = dst;
	//strcpy(recentCommands[1], dst);
//	copy(recentCommands[1], dst, n);
//}


//if the null sector hasnt been reached, move to next sector
//	if reached return the sector index
//if a while iteration happened, and no null sector where found,
//	write in first sector
...
//
  ...
  while(n > 0){
    ...
    //
	//if((int)input.buf[0] == 9)
	//cprintf(recentCommands[1]);
	//	cprintf("dont be talking shit\n");
...
{	
	...
	for(i = 0; i < n; i++){
		consputc(buf[i] & 0xff);
//		if(buf[0] == 'a' && buf[1] == 'b')
//			consputc(recentCommands[9][0] & 0xff);   
	}
	release(&cons.lock);
	ilock(ip);
