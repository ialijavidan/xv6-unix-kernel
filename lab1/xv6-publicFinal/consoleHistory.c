...
consoleintr(int (*getc)(void))
{
  ...
  while((c = getc()) >= 0){
    ...
    case C('H'): case '\x7f':  // Backspace
      ...
      break;
  case TAB:
    for(i = input.e; input.buf[i] != '\0' ; i++)
      buffer[i] = input.buf[input.e++];
    
    for(i = 0; i < strlen(*commandHistory) && !ifFound; i++)
      if(compare_strings(commandHistory[i], buffer) == 0){
        ifFound = 1;
        index = i;
      }
    ...
    if(index != -1){
      //show last command
      for(int i = strlen(buffer)-1; i < strlen(commandHistory[index]); i++){
